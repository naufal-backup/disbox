...
function initDatabase() {
  ...
  try { db.exec("CREATE INDEX IF NOT EXISTS idx_hash ON files(hash);"); } catch(_) {}
}
initDatabase();
...
function migrateLegacyJsonDataToSqlite(hash, data) {
  ...
  db.transaction(() => {
    ...
    const insertFile = db.prepare(\`INSERT INTO files (id, hash, path, parent_path, name, size, created_at, message_ids, is_locked, is_starred) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\`);
    for (const f of filesToSave) {
      const parts = f.path.split('/');
      const name = parts.pop();
      const parent_path = parts.join('/') || '/';
      insertFile.run(f.id || Math.random().toString(36).substring(7), hash, f.path, parent_path, name, f.size || 0, f.createdAt || Date.now(), JSON.stringify(f.messageIds || []), f.isLocked ? 1 : 0, f.isStarred ? 1 : 0);
    }
    if (pinHashToSync) db.prepare("INSERT INTO settings (hash, key, value) VALUES (?, 'pin_hash', ?) ON CONFLICT(hash, key) DO UPDATE SET value = excluded.value").run(hash, pinHashToSync);
  })();
}

function markMetadataDirty(hash) {
  db.prepare('INSERT INTO metadata_sync (hash, is_dirty, updated_at) VALUES (?, 1, ?) ON CONFLICT(hash) DO UPDATE SET is_dirty=1, updated_at=excluded.updated_at').run(hash, Date.now());
...
async function uploadMetadataToDiscord(hash) {
  ...
  if (res.ok) {
    ...
    await net.fetch(activeWebhookUrl, { method: 'PATCH', headers: {'Content-Type':'application/json'}, body: JSON.stringify({name: \`dbx: \${data.id}\`}) }).catch(()=>{});
  }
}
...
ipcMain.handle('get-latest-metadata-msgid', async (_, hash, webhookUrl) => {
  ...
  if (webhookUrl) {
    ...
    if (!webhookMsgId) {
      const msgRes = await net.fetch(normalizeUrl(webhookUrl) + '/messages?limit=100');
      if (msgRes.ok) {
        const msgs = JSON.parse(await msgRes.text());
        webhookMsgId = msgs.find(m => m.attachments?.some(a => a.filename.includes('metadata.json')))?.id;
      }
    }
  }
  const best = (webhookMsgId && meta?.last_msg_id) ? (BigInt(webhookMsgId) > BigInt(meta.last_msg_id) ? webhookMsgId : meta.last_msg_id) : (webhookMsgId || meta?.last_msg_id);
  return { lastMsgId: best, snapshotHistory: [] };
});
...
ipcMain.handle('load-metadata', async (_, hash) => {
  ...
  return rows.map(r => ({ ...r, messageIds: JSON.parse(r.message_ids), isLocked: !!r.is_locked, isStarred: !!r.is_starred }));
});

// Rest of essential handlers...
...
// Stub chunk upload for minimal working state
