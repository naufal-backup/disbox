...
function captureFrameFromBlob(blob) {
  return new Promise((resolve) => {
    ...
    video.onloadedmetadata = () => {
      // Seek ke detik 1 atau tengah, mana yang lebih kecil
    ...
    video.src = url;
    // Paksa load agar event terpicu meski partial data
    video.load();
  });
}
...
function FileThumbnail({ file, size = 32 }) {
  ...
  useEffect(() => {
    ...
    const loadThumb = async () => {
      ...
      try {
        await enqueueThumb(transferId, async () => {
          ...
          // Jika null (gagal capture), biarkan — icon default akan tampil
    ...
    return () => {
      ...
      // Hapus dari queue jika belum jalan
