...
export default function FileGrid() {
  const { api, files, currentPath, setCurrentPath, addTransfer, updateTransfer, removeTransfer, cancelTransfer, refresh, loading, movePath, copyPath, deletePath, bulkDelete, bulkMove, bulkCopy, uiScale } = useApp();
  ...
  const { processedFiles, processedDirs, folderSizes } = useMemo(() => {
    const fileList = [];
    ...
    files.forEach(f => {
      ...
      if (isInside && matchesSearch) {
        const fileDirStr = parts.slice(0, -1).join('/');
        if (q || fileDirStr === dirPath) {
          fileList.push(f);
        }
      }
      ...
      for (let i = 0; i < parts.length - 1; i++) {
        ...
        if (q) {
          const isDirInside = dirPath === '' || currentAcc.startsWith(dirPath + '/');
          if (isDirInside && dirName.toLowerCase().includes(q)) dirsMap.set(currentAcc, dirName);
  ...
  const handleFolderClick = (fullPath) => navigate('/' + fullPath);
  const handleFileClick = (file) => setPreviewFile(file);
  const handleDownloadClick = (file) => downloadFile(file);
  ...
  const handleUpload = async (selectedFiles) => {
    ...
    for (const file of selectedFiles) {
      ...
      try {
        if (nativePath) {
          await api.uploadFile({ nativePath, name: fileName }, uploadPath, (progress) => {
          ...
          await api.uploadFile({ buffer, name: fileName, size: buffer.byteLength }, uploadPath, (progress) => {
  ...
  return (
    <div className={`${styles.container} ${isDragOver ? styles.dragOver : ''} ${isSelectionMode ? styles.isSelectionMode : ''}`} style={{ '--zoom': zoom }} onDragOver={(e) => { e.preventDefault(); if (e.dataTransfer.types.includes('Files')) setIsDragOver(true); }} onDragLeave={() => setIsDragOver(false)} onDrop={(e) => { setIsDragOver(false); if (e.dataTransfer.files.length > 0) handleDropZone(e); }} onClick={() => { setContextMenu(null); if (!isSelectionMode) clearSelection(); }} onContextMenu={(e) => { if (e.target === e.currentTarget || e.target.classList.contains(styles.content) || e.target.classList.contains(styles.grid)) { e.preventDefault(); setContextMenu({ x: e.clientX / uiScale, y: e.clientY / uiScale, type: 'empty' }); } }}>
      ...
      {moveModal && <MoveModal id={moveModal.id} file={moveModal.path} paths={moveModal.paths} mode={moveModal.mode} onClose={() => { setMoveModal(null); clearSelection(); }} />}
